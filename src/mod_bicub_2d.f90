MODULE MOD_BICUB_2D
   ...
   !!-----------------------------------------------------------------
   !!
   !!         Method of interpolation : "bicubic 2D"
   !!         =======================================
   !!
   !!    M. Ballarotta, november 2015; Adapated from mod_bilin_2d.f90
   ...
   LOGICAL, PUBLIC, SAVE :: l_first_call_bicub = .TRUE. !: wether it is the 1st time that bicub_2d is called

   !! To be read into the netcdf file only at "l_first_call_bicub = .TRUE."
...
CONTAINS
   ...
   SUBROUTINE BICUB_2D(k_ew_per, X10, Y10, Z1, X20, Y20, Z2, cnpat)
      ...
      ALLOCATE ( slpx(ni1,nj1) , slpy(ni1,nj1) , slpxy(ni1,nj1) )


      !! Computation of partial derivatives:
      CALL PARTIAL_DERIV(k_ew_per, lon_in, lat_in, Z_in, slpx, slpy, slpxy)
      

      IF ( l_first_call_bicub ) THEN
      ...
      DO jj=1, ny2
         DO ji=1, nx2
            ...
            ELSE
               ...
               !! INTERPOLATION !
               Z2(ji,jj) = REAL(INTERP_BC(iP, jP, alpha, beta, Z_in, slpx, slpy, slpxy), 4)
      ...
      l_first_call_bicub = .FALSE.

      !!DEALLOCATE ( IMETRICS, RAB )

   END SUBROUTINE BICUB_2D
   ...
   FUNCTION INTERP_BC(iP, jP, xa, xb, Z_in, slpx, slpy, slpxy)
      ...
      REAL(8), DIMENSION(:,:), INTENT(in) :: slpx, slpy, slpxy

      REAL(8) :: INTERP_BC, &    !wup, &
         &       raa, rbb, xbm1, xam1, xb32, xa32, xxb, xxa, &
      ...
      w1  = xxb * xxa
      w2  = xxb * raa * xa32
      w3  = rbb * xb32 * raa * xa32
      w4  = rbb * xb32 * xxa

      w5  = xxb * xa * xam1*xam1
      w6  = xxb * raa *xam1
      w7  = rbb * xb32 * raa * xam1
      w8  = rbb * xb32 * xa * xam1*xam1
      w9  = xb * xbm1*xbm1 * xxa
      w10 = xb * xbm1*xbm1 * raa * xa32
      w11 = rbb * xbm1 * raa * xa32
      w12 = rbb * xbm1 * xxa

      w13 = xa * xam1*xam1 * xb * xbm1*xbm1
      w14 = raa * xam1 * xb * xbm1*xbm1
      w15 = raa * xam1 * rbb * xbm1
      w16 = xa * xam1*xam1 * rbb * xbm1
      ...
      INTERP_BC = ( &
         &  Z_in(i1,j1)*w1  +  Z_in(i2,j2)*w2  +  Z_in(i3,j3)*w3  +  Z_in(i4,j4)*w4  + &
         &  slpx(i1,j1)*w5  +  slpx(i2,j2)*w6  +  slpx(i3,j3)*w7  +  slpx(i4,j4)*w8  + &
         &  slpy(i1,j1)*w9  +  slpy(i2,j2)*w10 +  slpy(i3,j3)*w11 +  slpy(i4,j4)*w12 + &
         & slpxy(i1,j1)*w13 + slpxy(i2,j2)*w14 + slpxy(i3,j3)*w15 + slpxy(i4,j4)*w16   &
         & ) !/wup
      ! ELSE
      !    INTERP_BC = 666.
      ! ENDIF
         ...
         DO ji = 1, nxo
            ...
            DO WHILE (lagain)
               ...
               IF ((iP-1 == 0).OR.(jP-1 == 0).OR.(iP+1 == nxi+1).OR.(jP+1 == nyi+1)) THEN
                  PRINT *, 'WARNING, mod_bicub_2d.f90: bound problem => ',xP,yP,nxi,nyi,iP,jP
               ELSE
   ...
   SUBROUTINE LOCAL_COORD(xlam, xphi, xa, xb, ipb)
      ...
      IF ( niter >= itermax )   THEN
         ...
         !   PRINT *, ''
         !   PRINT *, 'LOCAL_COORD of mod_bicub_2d.f90: problematic region found!!!!'
